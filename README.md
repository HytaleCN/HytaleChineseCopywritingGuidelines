...
参见：
- [Aaran Xu 的中文排版指南](https://github.com/aaranxu/chinese-copywriting-guidelines)
...
- [Hytale 中文排版指南](#hytale-中文排版指南)
  ...
  - [全角和半角](#全角和半角)
    ...
    - [遇到包含数学概念的半角符号，保留半角符号](#遇到包含数学概念的半角符号保留半角符号)
    - [翻译原文句末如果没有句号，不要添加句号](#翻译原文句末如果没有句号不要添加句号)
  - [名词](#名词)
    - [专有名词使用正确的大小写](#专有名词使用正确的大小写)
    - [不要使用不地道的缩写](#不要使用不地道的缩写)
  - [变量](#变量)
    - [是否添加空格应以变量的实际值为准](#是否添加空格应以变量的实际值为准)
  - [争议](#争议)
    - [链接之间增加空格](#链接之间增加空格)
  - [推荐](#推荐)
    - [不重复使用标点符号](#不重复使用标点符号)
...
不明白什么是全角（全形）与半角（半形）符号？希望查看半角字符与全角字符的比较？请查看维基百科词条‘[全角和半角](http://zh.wikipedia.org/wiki/%E5%85%A8%E5%BD%A2%E5%92%8C%E5%8D%8A%E5%BD%A2)’。
...
### 遇到包含数学概念的半角符号，保留半角符号

正确：

> 你已被传送至 ({x, number}, {y, number}, {z, number})。
...
对于可以确定值类型的变量，应以其实际值为准进行排版。

正确：

> {username} 的游戏模式已被设置为{mode}
...
注意：如果变量确定为中文，则不应添加空格；只要变量允许包含英文或数字，则应按照[空格](#空格)中做出的规范考虑其两侧的空格。

## 争议

以下用法略带有个人色彩，既：无论是否遵循下述规则，从语法的角度来讲都是**正确**的。为了统一 Hytale 游戏文本风格，我们要求翻译工作者统一使用用法 1。
